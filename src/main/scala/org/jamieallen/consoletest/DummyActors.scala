...
object Bootstrap extends App {
  ...
  val system = ActorSystem("consoletest", config.getConfig("consoletest"))
  val savingsAccountProxy = system.actorOf(Props[SavingsAccountProxy].withDispatcher("my-pinned-dispatcher"), "savingsAccountProxy")
  val checkingAccountProxy = system.actorOf(Props[CheckingAccountProxy].withDispatcher("my-pinned-dispatcher"), "checkingAccountProxy")
  val moneyMarketAccountProxy = system.actorOf(Props[MoneyMarketAccountsProxy].withDispatcher("my-pinned-dispatcher"), "moneyMarketAccountProxy")
  val accountBalanceRetriever = system.actorOf(
    Props(new AccountBalanceRetriever(savingsAccountProxy, checkingAccountProxy, moneyMarketAccountProxy)).withDispatcher("my-pinned-dispatcher"),
  ...
  val receiver = system.actorOf(Props(new Actor() with ActorLogging {
    def receive = {
      case AccountBalances(cBalances, sBalances, mmBalances) =>
        ...
        for (s <- sBalances; (_, x) <- s) log.debug("Saving Balance: " + x)
  ...
  }).withDispatcher("my-pinned-dispatcher"), "accountBalanceReceiver")
}
